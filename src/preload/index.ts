...
const gitAPI = {
  ...
  discard: (repoRoot: string, filePath: string, kind: import('@shared/types/git').GitFileChangeKind) =>
    invoke('mt::git::discard', repoRoot, filePath, kind),
  ...
  saveCredentials: (hostKey: string, credentials: unknown) =>
    invoke('mt::git::save-credentials', hostKey, credentials as never),
  disconnect: (hostKey: string) => invoke('mt::git::disconnect', hostKey),
  isAuthenticated: (hostKey: string) => invoke('mt::git::is-authenticated', hostKey),
