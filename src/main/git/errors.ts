...
const USER_MESSAGES: Record<GitErrorCode, string> = {
  auth_failed: 'Your access token expired or is invalid. Update it in Connect settings.',
...
const mapErrorCode = (err: unknown, fallback: GitErrorCode): GitErrorCode => {
  ...
  if (message === 'auth_failed' || lower.includes('401') || lower.includes('authentication')) {
    return 'auth_failed'
  }
